...
class Decoder_module_upsampling(nn.Module):
        def __init__(self):
            ...
            self.DeConv_block1 = DeConv_block_upsampling(input_channels = 512, output_channels = 256, kernel_size=3, stride=1, padding=1, size=(5,15))
...
class Style_Encoder(nn.Module):
        ...
        def forward(self, x):
            ...
            x = self.Conv_block3(x)
            x = self.Conv_block4(x)
            out = self.Conv_block5(x) # 256 x 3 × 8 
            #x = x.view(x.size(0), -1)
            #mean = self.Fc_mean(x)
            #std = self.Fc_std(x)
            return out
...
class Convolutional_blend(nn.Module):
    ...
    def forward(self, masked_input, blend_gt):
        ...
        blend_feat = self.Style_Encoder_module(blend_gt) #mean and var
        
        concat = torch.cat([mask_feat, blend_feat], dim=1)
        #AdaIN_latent = AdaIN(mask_feat, blend_mean, blend_std)
        
        out = self.Decoder_module(concat)
        return out
