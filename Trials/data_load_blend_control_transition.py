...
class MotionLoader(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            if self.IsTrain == True:
                ...
                # trans_point -> 0~1 
                trans_point = ( random.randint(index,  index + masking_length- (blended_len // 2) ) - index) // (masking_length- (blended_len // 2))
                ...
                blend_zeros[: , trans_point : trans_point + blended_len] = blend_image[: , blend_index : blend_index + blended_len] 
            else:
                ...
                blend_index = ((index + index + masking_length) // 2) - (blended_len // 2)
                # trans_point -> 0~1 
                trans_point = ((((index + index + masking_length) // 2) - (blended_len // 2)) - index) // (masking_length- (blended_len // 2))
            ...
            return np.expand_dims(masked_input, axis=0), np.expand_dims(gt_image, axis=0), np.expand_dims(blend_zeros, axis=0), np.expand_dims(blend_image, axis=0), trans_point # it will (batch, 1, 69, 240)
