...
def main(args):
    ...
    for num_epoch in range(args.numEpoch):
        ...
        for iter, item in enumerate(train_dataloader):
            ...
            train_loss = loss_function(pred_affine, gt_blended_image.detach()) + loss_function(pred_recon, gt_image.detach())
        ...
        for iter, item in enumerate(valid_dataloader):
            ...
            val_loss = loss_function(pred_affine, gt_blended_image.detach()) + loss_function(pred_recon, gt_image.detach())
            #val_loss_root = loss_function(pred_affine[:, :, -7, :], gt_image[:, :, -7, :]) + loss_function(pred_affine[:, :, -6, :], gt_image[:, :, -6, :]) +loss_function(pred_affine[:, :, -5, :], gt_image[:, :, -5, :])
