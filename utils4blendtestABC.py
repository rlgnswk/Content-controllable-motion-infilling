...
class saveData():
    ...
    def save_result(self, pred, gt_image, blend_gt, gt_blended_image, blend_input, maksed_input, tail_input, epoch):
        ...
        tail_input = tail_input.detach().squeeze(1).permute(0,2,1).cpu().numpy()
        ...
        np.save(self.save_dir_validation + '/epoch_' + str(epoch) + "_tail_input", tail_input)
        ...
        for i in range(1): 
            plt.figure(figsize=(2,4))
            plt.matshow(tail_input[i], cmap=cmap)
            #plt.matshow(np.zeros(masked_input[i].shape), cmap=cmap)
            plt.clim(-100, 50)
            #plt.axis('off')
            plt.title("tail_input", fontsize=25)
            plt.savefig(self.save_dir_validation + '/epoch_'+ str(epoch) +'_tail_input'+str(i)+'.png')
