...
class Decoder_module_selfRef(nn.Module):
        def __init__(self):
            super(Decoder_module_selfRef, self).__init__()
            # input latent size 3 × 8 × 256  - HWC
            self.DeConv_block1 = DeConv_block(input_channels = 256*2, output_channels = 256, kernel_size=3, stride=2, padding=1, output_padding=0)
            self.DeConv_block2 = DeConv_block(input_channels = 256, output_channels = 256, kernel_size=3, stride=2, padding=1, output_padding=(0,1))
            self.DeConv_block3 = DeConv_block(input_channels = 256, output_channels = 128, kernel_size=3, stride=2, padding=1, output_padding=(1,1))
            self.DeConv_block4 = DeConv_block(input_channels = 128, output_channels = 64, kernel_size=3, stride=2, padding=1, output_padding=(0,1))
            #self.DeConv_block5 = DeConv_block(input_channels = output_channels, output_channels = output_channels, kernel_size=3, stride=1, padding=1, pooling=1)
...
## for self ref
class Convolutional_AE_SelfRef(nn.Module):
    def __init__(self):
        super(Convolutional_AE_SelfRef, self).__init__()
        # input sample of size 69 × 240
        self.Content_Incoder_module = Encoder_module()
        self.Stlye_Incoder_module = Encoder_module()
        
        self.Decoder_module = Decoder_module_selfRef()

    def forward(self, a, b):

        # a b
        content_latent_a = self.Content_Incoder_module(a)
        style_latent_b = self.Stlye_Incoder_module(b)
        out_style_B_Content_A = self.Decoder_module(torch.cat([content_latent_a, style_latent_b], dim=1)) # (B, C_c + C_s, H, W)
        
        content_latent_b = self.Content_Incoder_module(b)
        style_latent_a = self.Stlye_Incoder_module(a)
        out_style_A_Content_B = self.Decoder_module(torch.cat([content_latent_b, style_latent_a], dim=1)) # (B, C_c + C_s, H, W)

        return out_style_B_Content_A, out_style_A_Content_B, content_latent_a, style_latent_b, content_latent_b, style_latent_a 
